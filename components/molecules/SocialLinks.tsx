...
export default function SocialLinks({ size = 'default' }: SocialLinksProps) {
    const baseClass = size === 'large'
        ? 'text-gray-400 transition-all duration-300 hover:scale-110 flex items-center justify-center'
    ...
    return (
        <>
            <SocialIconLink
                ...
                className={`${baseClass} ${size === 'large' ? 'hover:text-white' : 'text-gray-500 hover:text-white'}`}
            />
            <SocialIconLink
                href="https://linkedin.com/in/aleor"
                ariaLabel="LinkedIn"
                icon={TECH.linkedin.icon}
                className={`${baseClass} ${size === 'large' ? 'hover:text-blue-500' : 'text-gray-500 hover:text-blue-500'}`}
            />
            <SocialIconLink
                href="mailto:alex03ortizperez@gmail.com"
                ariaLabel="Email"
                icon={TECH.gmail.icon}
                className={`${baseClass} ${size === 'large' ? 'hover:text-red-500' : 'text-gray-500 hover:text-red-500'}`}
                target="_self"
                rel=""
            />
        </>
